<?php
// Caminho: multi_pipeline/libraries/REST_Controller.php
...
class REST_Controller extends \CI_Controller
{
    ...
    public function __construct($config = 'rest')
    {
        ...
        if ($this->auth_override === false &&
            ...
            $rest_auth = $rest_auth ? strtolower($rest_auth) : '';
    ...
    public function _remap($object_called, $arguments = [])
    {
        ...
        $controller_method = $object_called . '_' . strtolower($this->request->method);
...
{
    // Verifica se emulação de método está habilitada e tenta obter o método a partir de POST ou cabeçalho
    $method = $this->config->item('enable_emulate_request') === true 
        ? ($this->input->post('_method') ?? $this->input->server('HTTP_X_HTTP_METHOD_OVERRIDE')) 
        : null;

    // Se nenhum método foi encontrado na emulação, usa o método da requisição
    $method = $method ? strtolower($method) : strtolower($this->input->server('REQUEST_METHOD'));

    // Verifica se o método é permitido e se o método de parsing correspondente existe
    return in_array($method, $this->allowed_http_methods) && method_exists($this, '_parse_' . $method)
        ? $method
        : 'get';
}
