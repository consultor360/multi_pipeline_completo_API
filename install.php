...
function multi_pipeline_install()
{
    ...
    add_expires_at_column_to_api_tokens_table($CI);
...
function create_api_tokens_table($CI)
{
    $fields = array(
        ...
        'expires_at' => array(
            'type' => 'DATETIME',
            'null' => TRUE // Expiração pode ser nula, indicando que o token não expira
        )
    );
...
function add_expires_at_column_to_api_tokens_table($CI)
{
    // Verifica se a coluna 'expires_at' já existe
    if (!$CI->db->field_exists('expires_at', 'multi_pipeline_api_tokens')) {
        $CI->dbforge->add_column('multi_pipeline_api_tokens', [
            'expires_at' => [
                'type' => 'DATETIME',
                'null' => TRUE
            ]
